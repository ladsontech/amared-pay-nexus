...
import { DollarSign, Wallet, TrendingUp, TrendingDown, Activity, Users, CheckCircle, Clock, AlertCircle, Building, Phone, Send, Banknote, Target, Calendar, BarChart3, X, Loader2 } from "lucide-react";
...
const OrgDashboard = () => {
  ...
  // Transform wallet transactions to dashboard format
  const recentTransactions = walletTransactions.slice(0, 5).map((transaction, index) => ({
    id: transaction.id,
    type: transaction.type === 'credit' ? 'Collection' : 'Payment',
    amount: Math.abs(transaction.amount),
    status: transaction.title?.includes('approved') ? 'approved' : 
            transaction.title?.includes('pending') ? 'pending' : 
            transaction.title?.includes('rejected') ? 'rejected' : 'completed',
    date: new Date(transaction.created_at).toISOString().split('T')[0],
    description: transaction.title || `${transaction.type} transaction`
  }));
  ...
  const teamMetrics = {
    totalStaff,
    activeStaff,
    monthlyBudget: 5000000, // This could be fetched from organization settings
    budgetUsed: Math.min(totalWalletBalance, 5000000)
  };

  // Calculate collections (sum of credit transactions)
  const totalCollections = walletTransactions
    .filter(t => t.type === 'credit')
    .reduce((sum, t) => sum + t.amount, 0);

  // Calculate petty cash (assuming it's a separate wallet or calculation)
  const pettyCashBalance = wallets.find(w => w.currency.name === 'UGX')?.balance || 0;
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm sm:text-base font-bold text-slate-900">
                  {loading ? (
                    ...
                    `UGX ${(totalCollections / 1000000).toFixed(1)}M`
                  )}
                </p>
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm sm:text-base font-bold text-slate-900">
                  {loading ? (
                    ...
                    `UGX ${(totalWalletBalance / 1000000).toFixed(1)}M`
                  )}
                </p>
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm sm:text-base font-bold text-slate-900">
                  {loading ? (
                    ...
                    `UGX ${(pettyCashBalance / 1000).toFixed(0)}K`
                  )}
                </p>
                <div className="flex items-center gap-1 text-xs text-slate-700 bg-blue-100 rounded-full px-2 py-1 w-fit">
                  <TrendingUp className="h-2.5 w-2.5" />
            ...
            <CardContent className="p-3 sm:p-4">
              <div className="space-y-2 sm:space-y-3">
                {loading ? (
                  ...
                  return (
                    <div key={transaction.id} className="group flex items-center justify-between p-2.5 sm:p-3 rounded-xl border border-blue-100 hover:border-blue-200 hover:shadow-sm transition-all duration-200 bg-blue-50/50 hover:bg-blue-100/50">
              ...
              <Card className="border-0 shadow-sm bg-gradient-to-br from-amber-50 to-amber-100/50">
                <CardHeader className="border-b border-amber-100 pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="bg-blue-600 text-white shadow-sm text-xs">{pendingApprovals}</Badge>
                  </div>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <div className="text-center p-2 rounded-lg bg-white/60 border border-indigo-100">
                      <div className="text-base sm:text-lg font-bold text-slate-900">
                        {loading ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : teamMetrics.totalStaff}
                      </div>
                      <div className="text-xs text-slate-600">Total Staff</div>
                    </div>
                    <div className="text-center p-2 rounded-lg bg-white/60 border border-indigo-100">
                      <div className="text-base sm:text-lg font-bold text-emerald-600">
                        {loading ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : teamMetrics.activeStaff}
                      </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs font-bold text-slate-900">
                        {loading ? (
                          ...
                          `${Math.round(teamMetrics.budgetUsed / teamMetrics.monthlyBudget * 100)}%`
                        )}
                      </span>
                    </div>
                    <div className="space-y-1">
                      <Progress value={teamMetrics.budgetUsed / teamMetrics.monthlyBudget * 100} className="h-2 bg-slate-100" />
                      <div className="flex justify-between text-xs text-slate-600">
                        <span>UGX {(teamMetrics.budgetUsed / 1000000).toFixed(1)}M</span>
                        <span>UGX {(teamMetrics.monthlyBudget / 1000000).toFixed(1)}M</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
